...
// -----------------------------------------------------------------------------
// Normal compilation:
//    gcc4 -lm -fopenmp main.c
// Optimized compilation:
//    gcc4 -lm -fopenmp -O3 main.c
// To run:
...
int main (int argc, char *argv[]) {
    ...
    #pragma omp parallel for private (i, j, min_dist, dist, min_loc)
    for (i = 0; i < ttl_points; i++) {
        min_dist = MAX_FLOAT;
        for (j = i + 1; j < ttl_points; j++) {
            dist = dist_m[i][j];
    ...
    for (loop = 0; loop < ttl_points - MIN_CLUSTERS; loop++) {
        ...
        #pragma omp parallel for private (i, min_dist, j, dist, min_loc)
