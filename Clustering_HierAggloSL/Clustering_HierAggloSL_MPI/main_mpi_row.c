...
int main (int argc, char *argv[]) {
    ...
    dist_m = (float **)malloc(block_size * sizeof(float *));
    ...
    for (i = 0; i < block_size; i++) {
        for (j = 0; j < ttl_points; j++) {
            dist_m[i][j] = E_DIST(points[i + block_start], points[j]);
        }
    }
    for (i = 0; i < block_size; i++) {
        dist_m[i][i + block_start] = MAX_FLOAT;
    }
    ...
    for (loop = 0; loop < ttl_points - MIN_CLUSTERS; loop++) {
        ...
        min_loc_bcast.i = block_start + min_loc;
        min_loc_bcast.j = min_row_loc[min_loc];
        ...
        if (clusters[min_loc_j] != min_loc_j) {
            if (pid == j_pid) {
                min_row_dist[min_loc_j - block_start] = MAX_FLOAT;
            }
            min_loc_j = clusters[min_loc_j];
            j_pid = EID_to_PID(min_loc_j, np, ttl_points);
        }
        ...
        for (i = 0; i < block_size; i++) {
            if ((dist_m[i][min_loc_i] < MAX_FLOAT) && \
                (dist_m[i][min_loc_j] < MAX_FLOAT)) {
                dist_m[i][min_loc_i] = MIN(dist_m[i][min_loc_i], \
                                           dist_m[i][min_loc_j]);
            } else {
                dist_m[i][min_loc_i] = MAX_FLOAT;
            }
        }
        ...
        if (pid == i_pid) {
            i = min_loc_i - block_start;
            // remove current minimum distance
            dist_m[i][min_loc_i] = MAX_FLOAT;
        }

        if (pid == j_pid) {
            i = min_loc_j - block_start;
            // remove current minimum distance
            dist_m[i][min_loc_i] = MAX_FLOAT;
            // remove the minimum row distance as well
            min_row_dist[i] = MAX_FLOAT;
        }
        ...
        if (pid == i_pid) {
            i = min_loc_i - block_start;
            min_row_dist[i] = minLocDst.distance;
            min_row_loc[i] = minLocDst.location;
        }
